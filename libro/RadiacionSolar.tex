...
en 1960 \citep{Liu.Jordan1960}. Para caracterizar la radiación solar
...
ese mes \citep{Aguiar.Collares-Pereira1992}.
...
la correlación para índices mensuales propuesta por Page \citep{Page1961}(Figura
...
\citep{Collares-Pereira.Rabl1979}(Figura \ref{fig:KtFDd}):\begin{equation}
F_{Dd}=\begin{cases}
0.99 & K_{Td}\leq0.17\\
...
diarios (unas 250 publicadas según algunos autores \citep{Miguel.Bilbao.ea2001}),
muchas de las cuales aplican sólo a un lugar concreto. El lector interesado
puede consultar las siguientes referencias \citep{Miguel.Bilbao.ea2001,Soler1990,Gopinathan.Soler1995,Macagnan1993}.
...
y la irradiación extra-atmosférica \citep{Collares-Pereira.Rabl1979}
(ecuación \ref{eq:Rd}):
...
empírico de \citep{Collares-Pereira.Rabl1979} para valores de irradiancia
...
\citep{Hay.McKay1985} (ecuación \ref{eq:DifusaHay}). Estos autores
...
son similares a los publicados por PVGIS \citep{Suri.Huld.ea2007}.
...
de la propagación del error \citep{Navidi2008}:

\begin{equation}
\delta_{G_{ef}}=\frac{\overline{G(0)}}{\overline{G(I)}}\cdot\left|\frac{dG(I)}{dG(0)}\right|\cdot\delta_{G(0)}\end{equation}


Es posible mostrar \citep{Perpinan2009} que, para sistemas de seguimiento
...
mediante la ecuación\footnote{Implementada en las funciones
  ...
  y
  \href{http://search.r-project.org/R/library/solaR/html/calcGef.html}{\texttt{calcGef}}
  de \texttt{solaR} \cite{Perpinan2012b}} \ref{eq:FTb} \citep{Martin.Ruiz2001}: 
\begin{equation}
...
% varía con el lugar en cuestión. No obstante, es posible mostrar \citep{Lorenzo2006c}
...
}de un sistema fotovoltaico. En \citep{Lorenzo2006c} se propone la
...
estáticos. Por ejemplo, en las referencias \citep{Caamano1998,Lorenzo2002}
