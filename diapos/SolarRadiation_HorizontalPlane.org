...
- Solar radiation data sources must *capture the long-term behaviour* (interannual variability) and be *representative of the specified location* (spatial variability).
...
** Hybrid methods
  ...
  - Spatial interpolation
    - *Inverse Distance Weighting (IDW)* ($d$ is the distance between locations $x_0$ and $x_i$)
      #+BEGIN_EXPORT latex
      \[
...
- Daily clearness index[fn:2] cannot exceed 1 (daily global irradiation cannot exceed extraterrestrial solar irradiation).
#+BEGIN_EXPORT latex
\[
  K_{dT} \leq 1
\]
...
- Clearness index must be higher than 0.03
#+BEGIN_EXPORT latex
  \[
...
- Measurements from a station should be compared with *nearby stations* (for example, using spatial interpolation)
- Comparison must be established with *aggregated values* (daily or monthly averages).

** Statistical Analysis of Deviations

- Statistical analysis of the deviations, $\mathbf{D}$, between Observations, $\mathbf{O}$, and a Model, $\mathbf{M}$ (or another set of observations):
