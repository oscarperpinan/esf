...
- Para evitar el cálculo con funciones sinusoidales se recurre a una transformación: *fasores*.
- Un fasor es un *número complejo* que representa una señal sinusoidal.
- El *módulo* del fasor es el *valor eficaz*, y el *argumento* es la *fase*.
- No se puede emplear cuando hay frecuencias diferentes en un mismo circuito.
...
y por tanto el $\cos(\theta)=1$ (la potencia aparente coincide con la
activa).
